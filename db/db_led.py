...
def add_led():
    try:
        ...
        s = 'insert into led (item_no, item_id, brand, model, quantity, price_per_unit, MRP) values(%s,%s,%s,%s,%s,' \
            '%s,%s) '
...
def delete():
    try:
        ...
        s = 'DELETE FROM led WHERE item_id = %s'
        value = (item_id,)
        cur.execute(s, value)
        mydb.commit()
...
def update():
    try:
        s = 'update led set item_id= %s, brand= %s, model= %s, quantity= %s, price_per_unit= %s, MRP= %s where ' \
            'item_no= %s '
        item_no = int(input('Enter Item no.: '))
        item_id = input('Enter Item id: ')
        brand = input('Enter Brand name: ')
