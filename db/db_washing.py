...
def add_washing():
    try:
        ...
        s = 'insert into washing_machine (item_no, item_id, brand, model, quantity, price_per_unit, MRP) values(%s,' \
            '%s,%s,%s,%s,%s,%s) '
...
def delete():
    try:
        ...
        s = 'DELETE FROM washing_machine WHERE item_id = %s'
        value = (item_id,)
        cur.execute(s, value)
        mydb.commit()
...
def update():
    try:
        s = 'update washing_machine set item_id= %s, brand= %s, model= %s, quantity= %s, price_per_unit= %s, ' \
